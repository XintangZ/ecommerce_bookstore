...
export class ChatService {
  ...
  private async handleOrderQuery(userId: string): Promise<string> {
    ...
    if (orders.length > 0) {
      const pendingOrderLink = `<a href='FRONTEND_URL/orders?status=Pending'>pending ${pendingCount > 1 ? 'orders' : 'order'
        }</a>`;
  ...
  private async checkBookAvailability(bookTitle: string): Promise<string> {
    try {
      ...
      if (book.stock > 0) {
        return `Good news! "${bookTitleWithLink}" by ${book.author} is available. We currently have ${book.stock
